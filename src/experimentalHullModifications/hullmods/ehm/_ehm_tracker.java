	...
	private static class fleetTrackerScript implements EveryFrameScriptWithCleanup {
		...
		private lyr_shipTracker getShipTracker(ShipAPI ship) {
			...
			if (shipTracker == null) {
				...
				if (lyr_ehm.settings.getLogTrackerInfo()) logger.info(logPrefix+"ST-"+memberId+": Ship Tracker initialized");
			}
		...
		private void removeShipTracker(ShipAPI ship) {
			...
			if (lyr_ehm.settings.getLogTrackerInfo()) logger.info(logPrefix+"ST-"+memberId+": Ship Tracker terminated");
		}
		//#endregion
